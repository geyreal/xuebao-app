<template>
  <view class="checkin-page">
    ...
    <view class="stats-card">
      ...
      <view class="stat-item">
        <text class="stat-value">{{ stats.totalChecked }}</text>
        <text class="stat-label">累计打卡</text>
      </view>
      <view class="stat-divider" />
      <view class="stat-item">
        <text class="stat-value">{{ Math.round(stats.completionRate * 100) }}%</text>
    ...
    <view class="section calendar-section">
      ...
      <view class="calendar-grid">
        <view class="weekday-row">
          <text v-for="day in weekDays" :key="day" class="weekday-cell">{{ day }}</text>
        </view>
      ...
      <view v-for="task in tasks" :key="task.id" class="task-card">
        <view class="task-header flex-between">
          <view class="task-info">
            ...
            <text class="task-subject" :style="{ color: getSubjectColor(task.subjectId) }">
              {{ task.subjectName }}
            </text>
          </view>
          <view
          ...
          >
            <text class="checkin-btn-text">{{ task.isChecked ? '已打卡' : '打卡' }}</text>
          </view>
        </view>
        ...
        <view class="task-footer flex-between">
          <text class="task-deadline">
            {{ task.deadline ? '截止：' + task.deadline.slice(0, 10) : '无截止时间' }}
          </text>
...
import { getCheckinTasks, getCheckinStats, getCheckinCalendar } from '@/api/checkin'
...
const stats = ref<CheckinStats>({ consecutiveDays: 0, totalChecked: 0, completionRate: 0 })
...
async function loadData() {
  try {
    const [taskRes, statsRes, calendarRes] = await Promise.all([
      getCheckinTasks({ page: 1, pageSize: 20 }),
      getCheckinStats(),
      getCheckinCalendar({ year: currentYear.value, month: currentMonth.value }),
    ])
    tasks.value = taskRes.list
    stats.value = statsRes
    checkedDates.value = calendarRes
  } catch (e) {
...
async function loadCalendar() {
  try {
    checkedDates.value = await getCheckinCalendar({ year: currentYear.value, month: currentMonth.value })
  } catch (e) {}
}
...
function handleCheckin(task: CheckinTask) {
  ...
  task.isChecked = true
  if (task.checkedCount !== undefined) task.checkedCount++
...
.date-cell.checked .date-text {
  color: #4A90D9;
  font-weight: 600;
}
.date-cell.today .date-text {
  background: #4A90D9;
  color: #FFFFFF;
  width: 48rpx;
  height: 48rpx;
...
.checkin-btn {
  ...
  box-shadow: 0 2rpx 8rpx rgba(74, 144, 217, 0.3);
}
.checkin-btn.checked {
  background: #E4E7ED;
  box-shadow: none;
